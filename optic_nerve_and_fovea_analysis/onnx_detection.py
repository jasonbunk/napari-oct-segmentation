...
from definitions_and_config import RetinaLayers
...
@njit
def make_mask_for_rpe_det(annos2d, top_mask_w, value_of_nonzero:float):
    """
    for each image row:
    ...
    """
    rpe_anno = np.equal(annos2d, RetinaLayers.RPE.value)
...
def make_mask_for_det_left_of_rpe(annos2d, rpe_det, which_layer_value, top_mask_w, rpe_width, value_of_nonzero:float):
    ...
    """
    ilm_anno = np.equal(annos2d, which_layer_value)
    mask = np.zeros(ilm_anno.shape, dtype=np.float64)
    for ii in range(mask.shape[0]):
        if np.any(ilm_anno[ii,:]): # if any pixel in this row was annotated
            mask[ii,:] = ilm_anno[ii,:].astype(np.float64) * value_of_nonzero * 5. # set mask row to user annotation, so that detection should occur within masked pixels
        else:
...
def _compute_masked_irx_detection_with_user_annos(preds:np.ndarray, userannos:np.ndarray, detect_these_layers:tuple, ilm_nms_w:int, ilm_top_mask:int, rpe_top_mask:int, max_slope:int):
    ...
    rpe_idx = RetinaLayers.RPE.value - 1
    ilm_idx = RetinaLayers.ILM.value - 1
    ret[:,rpe_idx] = smooth_det(get_sloped_seam(preds[rpe_idx,:,:].astype(np.float64) - make_mask_for_rpe_det(userannos, rpe_top_mask, 1e5), max_slope))
    ret[:,ilm_idx] = smooth_det(get_sloped_seam(preds[ilm_idx,:,:].astype(np.float64) - make_mask_for_det_left_of_rpe(userannos, ret[:,rpe_idx], 1+ilm_idx, ilm_top_mask, ilm_nms_w, 1e5), max_slope))
...
def _compute_masked_sequential_detection_with_user_annos(preds:np.ndarray, userannos:np.ndarray, ilm_nms_w:int, rpe_top_mask:int, ilm_top_mask:int, max_slope:int):
    ...
    rpe_idx = RetinaLayers.RPE.value - 1
    ilm_idx = RetinaLayers.ILM.value - 1
    ipn_idx = RetinaLayers.IPLINL.value - 1
    gci_idx = RetinaLayers.GCLIPL.value - 1
    rng_idx = RetinaLayers.RNFLGCL.value - 1
    ret[:,rpe_idx] = smooth_det(get_sloped_seam(preds[rpe_idx,:,:].astype(np.float64) - make_mask_for_rpe_det(userannos, rpe_top_mask, 1e5), max_slope))
    ret[:,ilm_idx] = smooth_det(get_sloped_seam(preds[ilm_idx,:,:].astype(np.float64) - make_mask_for_det_left_of_rpe(userannos, ret[:,rpe_idx], 1+ilm_idx, ilm_top_mask, ilm_nms_w, 1e5), max_slope))
